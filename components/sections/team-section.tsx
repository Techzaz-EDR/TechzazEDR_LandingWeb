            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                    {team.map((member, index) => (
                        <div
                            key={index}
                            className="p-6 rounded-[30px] bg-card border border-border text-center hover:border-primary hover:shadow-[inset_0_0_0_1px_rgb(94,27,181),0_0_30px_-10px_rgba(94,27,181,0.5)] transition-all duration-300 group overflow-hidden"
                        >
                            <div className="transition-transform duration-300 group-hover:scale-110">
                                <div className={`w-20 h-20 rounded-full ${member.bgColor} mx-auto mb-4 flex items-center justify-center`}>
                                    <img
                                        src={`https://api.dicebear.com/9.x/${member.collection}/svg?seed=${member.avatar}&backgroundColor=transparent`}
                                        alt={member.name}
                                        className="w-16 h-16"
                                    />
                                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="p-8 rounded-xl bg-card border border-border hover:border-primary transition-all duration-300">
